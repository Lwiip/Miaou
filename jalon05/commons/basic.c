...
void replace_str(char * str, char * origine, char * replacement, char * out){
    ...
    if(p = strstr(str, origine)){
        strncpy(buffer, str, p - str);


        buffer[p - str] = '\0'; //on termine la string

        sprintf(buffer + (p - str), "%s%s", replacement, p + strlen(origine));

        sprintf(out, "%s", buffer);

        replace_str(out, origine, replacement, out);
    } else {
        sprintf(out, "%s", str);
    }
}
