...
#define PORT 7000 // The port which is communicate with server
#define BACKLOG 10
...
int main ()
{
    ...
    while(success == 0)
    {
        ...
        if ((nsockfd = accept(sockfd, (struct sockaddr *)&addr_remote, &sin_size)) == -1){
            ...
            printf ("[server] server has got connect\n");
        }
        /* Child process */
        if(!fork())
        {
            ...
            {
                int offset = 0;
                while ((sent = send(nsockfd, sdbuf + offset, f_block_sz, 0)) > 0 || (sent == -1 && errno == EINTR) ) {
                        if (sent > 0) {
                            offset += sent;
                            nbytes -= sent;
                        }
